/*
  ...
  1. New Tables
    ...
    - `airdrop_tokens`
      ...
      - `currency_code` (text)
      - `issuer_address` (text)
      - `distribution_method` (text) - fixed, wallet_balance_percent, etc.
      - `amount` (numeric) - For fixed distribution
      - `min_amount` (numeric) - For random range
      - `max_amount` (numeric) - For random range
      - `balance_percent` (numeric) - For balance percentage methods
      - `source_token_currency` (text) - For token_balance_ratio
      - `source_token_issuer` (text) - For token_balance_ratio
...
CREATE TABLE IF NOT EXISTS airdrop_tokens (
  ...
  distribution_method text NOT NULL DEFAULT 'fixed',
  amount numeric,
  min_amount numeric,
  max_amount numeric,
  balance_percent numeric,
  source_token_currency text,
  source_token_issuer text,
